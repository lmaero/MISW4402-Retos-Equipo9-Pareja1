...
// Configuraciones para Luminosidad
#define LDR A0 // LDR connected to A0 pin
...
void loop()
{
  ...
  //El mensaje que se envía es de la forma {"value": x}, donde x es el valor de temperatura o humedad
   // Lee datos de luminosidad del LDR
  int V = analogRead(LDR);
  float luminosity = ((long)V*A*10)/((long)B*Rc*(1024-V));
  ...
  String jsonLuminosity = "{\"value\": "+ String(luminosity) + "}";
  char payload3[jsonLuminosity.length()+1];
  jsonLuminosity.toCharArray(payload3, jsonLuminosity.length()+1);
