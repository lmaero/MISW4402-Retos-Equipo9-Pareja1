...
float getLightIntensity(int ldrValue) {
  const float NODE_MCU_VOLTAGE = 3.3;
  const float MAX_ANALOG_VALUE = 1023.0;
  float voltage = ldrValue * (NODE_MCU_VOLTAGE / MAX_ANALOG_VALUE);  // Convert analog value to voltage

  // Calculate the resistance of the LDR using a voltage divider formula
  float circuitResistance = 10000;
  float ldrResistance = (circuitResistance * voltage) / (NODE_MCU_VOLTAGE - voltage);  // Using a 10kΩ resistor in series with LDR

  // Constants based on GL55 series datasheet
  const int DARK_RESISTANCE = 500;
  const int LIGHT_RESISTANCE = 10000;
  const int MIN_PERCENTAGE = 0;
  const int MAX_PERCENTAGE = 100;
  float lightIntensity = map(ldrResistance, DARK_RESISTANCE, LIGHT_RESISTANCE, MIN_PERCENTAGE, MAX_PERCENTAGE);
...
void loop() {
  ...
  float lightIntensity = getLightIntensity(ldrValue);
